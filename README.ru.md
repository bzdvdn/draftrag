...
Подробная capability-таблица: [docs/ru/vector-stores.md](docs/ru/vector-stores.md).
...
func main() {
    ...
    pipeline, err := draftrag.NewPipelineWithOptions(
...
pipeline, err := draftrag.NewPipeline(store, resilientLLM, resilientEmbedder)
if err != nil {
    log.Fatal(err)
}
```
...
pipeline, err := draftrag.NewPipelineWithOptions(store, llm, embedder,
    draftrag.PipelineOptions{
        Hooks: &myHooks{},
    },
)
...
pipeline, err := draftrag.NewPipeline(store, llm, cached)
if err != nil {
    log.Fatal(err)
}
```
...
Документация:
- [docs/ru/compatibility.md](docs/ru/compatibility.md) — политика совместимости и поддержки
- [docs/ru/production.md](docs/ru/production.md) — production checklist + runbook
- [docs/ru/getting-started.md](docs/ru/getting-started.md) — начало работы
