...
- В миграциях зафиксировать детерминированные имена индексов и стратегию смены `IndexMethod` (в спеке уже: drop+create без `CONCURRENTLY`).
- Для `ivfflat`/`hnsw` явно описать, где живут runtime-параметры (`probes`, `ef_search`): либо через SQL `SET LOCAL`, либо через отдельные query hints/options, чтобы не “прятать” критичную производительность в неявные настройки.
