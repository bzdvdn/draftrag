...
### Публичный API: options pattern

Для публичных конструкторов в `pkg/draftrag` используйте единый паттерн:

- `NewX(...required, opts XOptions)` — `XOptions` это struct; zero-values означают “использовать дефолты”.
- Если опции не нужны — конструктор НЕ принимает options параметр.
- Если у компонента логически две группы опций (например, “подключение” и “runtime лимиты”) — они объединяются в ОДИН options-контейнер (с вложенными секциями вроде `Runtime`), а старые формы (если были) остаются как `// Deprecated:` thin-wrapper с миграцией в docs.
- Исключения допускаются только при явном обосновании и должны быть закреплены guardrail-тестом (allowlist).
