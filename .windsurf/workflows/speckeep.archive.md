...
Сначала откройте и прочитайте файл ".speckeep/templates/prompts/archive.md" (обязательно). Затем строго следуйте его разделу "Output expectations", включая финальную строку `Готово к: ...` (без замены на «Следующий шаг», без альтернативных фаз, без выдуманных флагов).

Команда: `/speckeep.archive [request]`

Цепочка workflow: constitution → spec → inspect → plan → tasks → implement → verify → archive. Не пропускайте фазы и не забегайте вперёд.

Используйте этот workflow, когда запрос явно относится к фазе "archive" или команде /speckeep.archive.
...
- Scripts для выполнения (запускать через shell):
  - `./.speckeep/scripts/check-archive-ready.sh`
