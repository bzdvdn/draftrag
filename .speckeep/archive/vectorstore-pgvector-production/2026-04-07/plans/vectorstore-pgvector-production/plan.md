...
Inputs: `.draftspec/specs/vectorstore-pgvector-production/spec.md` и узкий контекст текущей реализации pgvector.
Outputs: `.draftspec/plans/vectorstore-pgvector-production/plan.md`, `.draftspec/plans/vectorstore-pgvector-production/data-model.md`.
...
- Data model меняется: таблица чанков расширяется полями `metadata`, `created_at`, `updated_at` (V2) и добавляются индексы на `parent_id` и `(parent_id, position)`. См. `.draftspec/plans/vectorstore-pgvector-production/data-model.md`.
