...
- `BMFinaKK` присутствует в 5 Go-файлах; в архивных `.draftspec/`-файлах менять не нужно.
...
- RQ-002 После переименования `go build ./...` ДОЛЖЕН завершаться без ошибок; старое имя `BMFinaKK` НЕ ДОЛЖНО присутствовать ни в одном Go-файле (кроме архивных `.draftspec/`).
...
- Обновление архивных `.draftspec/` файлов.
...
- Архивные `.draftspec/` файлы не компилируются Go-компилятором — их обновлять не нужно.
