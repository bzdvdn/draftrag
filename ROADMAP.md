...
### ChromaDB: гибридный поиск и миграции
ChromaDB сейчас поддерживает только базовый векторный поиск. Для полноценного использования нужны:
- Гибридный поиск (Chroma не поддерживает BM25 нативно — требуется кастомная реализация или отказ от фичи)
- Миграции: `CreateCollection`, `DeleteCollection`, `CollectionExists`

**Что нужно:**
- Решить: реализовать BM25 через внешний индекс или отказаться от hybrid для ChromaDB
- Добавить функции управления коллекциями в публичный API
...
Сейчас документация разбросана по README и тестам. Нужны структурированные docs и рабочие examples.

**Что нужно:**
- `docs/` с полным покрытием: getting-started, concepts, pipeline-api, stores, embedders, llm, chunking, advanced
...
**Weaviate** — GraphQL-интерфейс, modular AI integrations
